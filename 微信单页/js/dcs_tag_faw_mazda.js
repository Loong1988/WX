...
// Version: 9.2b  
// Add Sleep for Firefox; Add X*Y for clickheat; Add Page Double Collect;
function Sleep() {
...
function WebTrends(){
	...
	this.DCSext={};
	// Custom Params;
	this.domain="sdc.faw-mazda.com";
	this.dcsid="dcstezfq4100000827tjhafv5_8g4j";
	this.fpcdom=".faw-mazda.com";
	// Custom Params;
	this.timezone=8;
	this.onsitedoms="";
...
WebTrends.prototype.dcsPos=function(evt){
		var x = evt.clientX;
		var y = evt.clientY;
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href){
		    ...
		    if ( this.dcsIsOnsite(hn) && pr.toLowerCase() !="javascript:" && !(e.hash&&(e.hash!="")&&(e.hash!="#"))){
			    ...
			    setTimeout(Sleep, 0);
				this.dcsMultiTrack("WT.event","pageLink:"+e.href,"WT.dl","24","WT.nv",this.dcsNavigation(evt),"WT.pos",this.dcsPos(evt));
			    this.WT.event=this.WT.dl=this.WT.nv=this.WT.pos="";
		    }
		}
	}
}
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href&&e.protocol){
			...
			if (e.protocol.toLowerCase()=="javascript:"){
				setTimeout(Sleep, 0);
				this.dcsMultiTrack("WT.event","JavaScript:"+e.innerHTML,"WT.dl","22","WT.nv",this.dcsNavigation(evt),"WT.pos",this.dcsPos(evt));
				this.WT.event=this.WT.cl=this.WT.nv=this.WT.pos="";
			}
		}
	}
}
		...
		for (var j=0;j<tags.length;j++){
			...
			if (type&&((type=="submit")||(type=="image")||(type=="button")||(type=="reset"))||((type=="text")&&((evt.which||evt.keyCode)==13))){
				...
				if (uri&&ttl&&(evt.keyCode!=9)){
					setTimeout(Sleep, 0);
					this.dcsMultiTrack("WT.event","FormButton2:"+ttl,"WT.dl",id,"WT.nv",this.dcsNavigation(evt),"WT.pos",this.dcsPos(evt));
				}
				this.WT.event=this.WT.dl=this.WT.nv=this.WT.pos="";
				break;
			}
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href){
		    ...
		    if (this.dcsIsOnsite(hn)&&e.hash&&(e.hash!="")&&(e.hash!="#")){
			    ...
			    var pth=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";
				setTimeout(Sleep, 0);
			    this.dcsMultiTrack("WT.event","Anchor:"+e.hash.substring(1,e.hash.length),"WT.dl","21","WT.nv",this.dcsNavigation(evt),"WT.pos",this.dcsPos(evt));
			    this.WT.event=this.WT.dl=this.WT.nv=this.WT.pos="";
		    }
		}
	}
	...
	if (evt){
		...
		if (e.href){
		    ...
		    if ((hn!="")&&e.protocol&&(e.protocol.indexOf("http")!=-1)){
			    ...
			    var pth=e.pathname?((e.pathname.indexOf("/")!=0)?"/"+e.pathname:e.pathname):"/";
				setTimeout(Sleep, 0);
			    this.dcsMultiTrack("WT.event","ImageMap:"+ttl,"WT.dl","30","WT.nv",this.dcsNavigation(evt),"WT.pos",this.dcsPos(evt));
			    this.WT.event=this.WT.dl=this.WT.nv=this.WT.pos="";
		    }			
		}			
	}
